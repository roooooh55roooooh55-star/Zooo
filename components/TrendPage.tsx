...
const TrendPage: React.FC<TrendPageProps> = ({ onPlayShort, onPlayLong, excludedIds }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // نجلب قائمة كبيرة لنتمكن من اختيار 6 شورتس و 4 طويل منها
  ...
  const filteredTrends = useMemo(() => {
    // 1. استبعاد الفيديوهات المخفية
    const pool = rawTrends.filter(v => !excludedIds.includes(v.id || v.video_url));

    // 2. تقسيم الفيديوهات حسب النوع وترتيبها حسب المشاهدات
    ...
    // 3. اختيار 6 شورتس و 4 طويل
    const top6Shorts = allShorts.slice(0, 6);
    const top4Longs = allLongs.slice(0, 4);

    // 4. الدمج في قائمة واحدة نهائية مكونة من 10 فيديوهات
    return [...top6Shorts, ...top4Longs].sort((a, b) => (b.views || 0) - (a.views || 0));
  }, [rawTrends, excludedIds]);

  if (loading) return <div className="p-10 text-center text-gray-500 animate-pulse">جاري تحليل بيانات الترند الحقيقية...</div>;
    ...
    <div className="flex flex-col gap-8 pb-32">
      <div className="flex items-center justify-between border-b border-red-600/20 pb-4">
        <div className="flex flex-col">
           ...
           <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest mt-1">Top 10: 6 Shorts + 4 Originals</p>
        </div>
        <div className="flex gap-1.5 items-center">
           <span className="text-[10px] text-red-600 font-black animate-pulse">LIVE DATA</span>
          ...
          return (
            ...
            >
              <div className="aspect-video relative bg-black overflow-hidden">
                ...
                {/* علامة النوع */}
                <div className={`absolute top-5 left-5 px-4 py-1.5 rounded-full text-[10px] text-white border border-white/10 font-black uppercase tracking-widest z-20 backdrop-blur-md ${isShort ? 'bg-red-600/60' : 'bg-blue-600/60'}`}>
                  {isShort ? 'Shorts Rank' : 'Series Rank'}
                </div>
              </div>
              
              <div className="p-7 flex flex-col gap-4 relative z-20 bg-[#0f0f0f]/60 backdrop-blur-sm">
                <div className="flex justify-between items-start gap-4">
                  <h3 className="font-black text-xl line-clamp-1 leading-tight tracking-tight text-right flex-grow">{video.title}</h3>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-5">
                    ...
                    <span className="flex items-center gap-2 text-sm text-blue-400 font-black">
                      ...
                      {formatBigNumber(video.views || 0)} مشاهدة
                    </span>
                  </div>
                  <div className={`w-2 h-2 rounded-full ${isShort ? 'bg-red-600 animate-ping' : 'bg-blue-600 animate-pulse'}`}></div>
                </div>
              </div>
            </div>
          );
