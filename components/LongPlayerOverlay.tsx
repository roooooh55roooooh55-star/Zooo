      ...
      <div className={`relative flex flex-col transition-all duration-500 p-2 ${isFullScreen ? 'h-full w-full bg-black p-0' : 'h-[40vh] mt-4'}`}>
        <div className={`absolute px-6 flex justify-between items-center z-[220] transition-all duration-500 ${
          ...
          <button 
            ...
            className={`p-3 bg-black/60 backdrop-blur-xl rounded-2xl border-2 border-red-500 text-red-500 shadow-[0_0_20px_red] active:scale-90 transition-all ${isFullScreen ? 'rotate-90' : ''}`}
          >
        ...
        <div className="flex-grow flex flex-col p-6 gap-6 overflow-hidden">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between text-[11px] font-black opacity-70">
              <span className="text-blue-400">{formatBigNumber(stats.views)} شاهدوا الكابوس</span>
              <span className="text-red-500">{formatBigNumber(stats.likes)} تفاعلوا</span>
            </div>
            <div className="grid grid-cols-4 gap-4">
              <button onClick={onLike} className={`py-4 rounded-[1.5rem] border-2 transition-all flex justify-center items-center active:scale-125 ${isLiked ? 'bg-red-600 border-red-400 text-white shadow-[0_0_25px_red]' : 'bg-red-600/10 border-red-600 text-red-600 shadow-[0_0_15px_red]'}`}>
              ...
              <button onClick={onDislike} className={`py-4 rounded-[1.5rem] border-2 transition-all flex justify-center items-center active:scale-125 ${isDisliked ? 'bg-blue-600 border-blue-400 text-white shadow-[0_0_25px_blue]' : 'bg-blue-600/10 border-blue-600 text-blue-600 shadow-[0_0_15px_blue]'}`}>
              ...
              <button onClick={onSave} className={`py-4 rounded-[1.5rem] border-2 transition-all flex justify-center items-center active:scale-125 ${isSaved ? 'bg-yellow-500 border-yellow-300 text-white shadow-[0_0_25px_yellow]' : 'bg-yellow-500/10 border-yellow-500 text-yellow-500 shadow-[0_0_15px_yellow]'}`}>
              ...
              <button onClick={() => setIsMuted(!isMuted)} className={`py-4 rounded-[1.5rem] border-2 transition-all flex justify-center items-center ${isMuted ? 'bg-red-600/30 border-red-500 text-red-500' : 'bg-white/5 border-white/10 text-white'}`}>
