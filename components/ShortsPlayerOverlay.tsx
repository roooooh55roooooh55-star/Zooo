          ...
          return (
            <div key={`${videoId}-${idx}`} className="h-full w-full snap-start relative bg-black flex items-center justify-center overflow-hidden">
              ...
              <div className="absolute bottom-32 left-6 flex flex-col items-center gap-6 z-30">
                <button onClick={() => setIsAutoPlay(!isAutoPlay)} className={`w-14 h-14 flex items-center justify-center rounded-full border-2 transition-all text-[9px] font-black ${isAutoPlay ? 'bg-green-600/50 border-green-400 text-white shadow-[0_0_20px_green]' : 'bg-red-600/50 border-red-400 text-white shadow-[0_0_20px_red]'}`}>AUTO</button>
                ...
                <button onClick={() => onDislike(videoId)} className={`p-4 rounded-full border-2 transition-all ${isDisliked ? 'bg-red-600 border-red-400 text-white shadow-[0_0_20px_red]' : 'bg-black/40 border-white/20 text-white'}`}><svg className="w-6 h-6 rotate-180" fill={isDisliked ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path d="M14 9V5a3 3 0 00-3-3l-4 9v11h11.28a2 2 0 002-1.7l1.38-9a2 2 0 00-2-2.3zM7 22H4a2 2 0 01-2-2v-7a2 2 0 012-2h3"/></svg></button>
                <button onClick={() => setIsMuted(!isMuted)} className={`p-4 rounded-full border-2 transition-all ${isMuted ? 'bg-red-600/40 border-red-500 text-white shadow-[0_0_15px_red]' : 'bg-black/40 border-white/20 text-white'}`}>{isMuted ? <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path d="M5.586 15L4 16.586V7.414L5.586 9H10l5-5v16l-5-5H5.586zM17 9l4 4m0-4l-4 4"/></svg> : <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15L4 16.586V7.414L5.586 9H10l5-5v16l-5-5H5.586z"/></svg>}</button>
              </div>
